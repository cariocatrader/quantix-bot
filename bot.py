...
# =========================
# ANALYZE - TENDÊNCIA COM COINGECKO (SÓ PARA DIREÇÃO)
# =========================
def analyze(coin_id):
    global last_call
    ...
    try:
        # CoinGecko só para análise de tendência, não para candle 1m
...
def get_result_raw(direction, coin_id, target_time_dt):
    # Verificar se a moeda é suportada no Bybit
    if coin_id not in COIN_IDS_BYBIT:
    ...
    while True:
        ...
        try:
            ...
            params = {
                ...
                "from": start_time_ms,
                "to": end_time_ms,
                "limit": 100  # ultrapassa o que precisa
            }
            ...
            # converter target_time_dt para UTC
...
def menu_paridades(chat_id):
    ...
    # key vem de COIN_IDS_BYBIT (ex: "btc", "eth")
...
def send_result(chat_id, coin_id, direction, entry, gale, status):
    ...
    # SYMBOLS agora usa a mesma chave: "btc", "eth", etc.
    text = f"""
🎯 RESULTADO
...
"""
    ...
    # --- RESULTADO APÓS 65s (USANDO BYBIT 1m)
