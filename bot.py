...
# 2. Apenas pares que a Kraken realmente aceita no OHLC (ex: XBTUSD)
KRAKEN_PAIRS = {
    "btc": "XBTUSD",
}

SYMBOLS = {
    "btc": "₿ BTC/USD",   # só BTC no menu por enquanto
}
...
def analyze_kraken(coin_id):
    if coin_id not in KRAKEN_PAIRS:
        print("❌ analyze_kraken: moeda não suportada na Kraken")
        return "COMPRA"

    pair = KRAKEN_PAIRS[coin_id]
    ...
    for tentativa in range(3):
        try:
            ...
            if not result or pair not in result:
                print("⚠️ Kraken: par não encontrado no result:", pair, " | Tente outro nome no .json")
                time.sleep(3)
                continue
...
# =========================
# GET_RESULT_RAW - KRAKEN 1m (AJUSTADO, TIMEOUT 40s)
# =========================
def get_result_raw(direction, coin_id, target_time_dt):
    ...
    # Converta para UTC e arredonde para o minuto ZERADO
    ...
    max_wait = 40.0    # TIMEOUT REDUZIDO
    intervalo = 5      # 5s entre tentativas
    ...
    while True:
        ...
        if elapsed > max_wait:
            print(f"⏰ Timeout {max_wait}s atingido para {target_time_dt.strftime('%H:%M')}, tratando como LOSS.")
            return "LOSS"
        ...
        try:
            ...
            result = data.get("result", {})
            if not result or pair not in result:
                print("⚠️ Kraken: par não encontrado no result (possível nome errado):", pair)
                time.sleep(intervalo)
                continue
            ...
            # Procurar o candle pelo timestamp exato
            candle_found = None
            for k in ohlc:
                try:
                    if isinstance(k, list) and len(k) >= 5:
                        candle_time_s = int(k[0])
    ...
    print(
        f"⏰ Timeout {max_wait}s atingido buscando candle 1m {target_time_dt.strftime('%H:%M')} na Kraken, tratando como LOSS."
    )
    return "LOSS"
...
def menu_paridades(chat_id):
    ...
    # Só BTC por enquanto (XBTUSD)
    kb.add(
        InlineKeyboardButton(
            SYMBOLS["btc"],
            callback_data="par_btc",
        )
    )
...
@bot.callback_query_handler(func=lambda c: c.data == "par_btc")
def par_btc(c):
    bot.answer_callback_query(c.id)
    chat_id = c.message.chat.id
...
print("🚀 QUANTIX PRO ATIVO (Kraken 1m, sem Gale 1, até 40s de espera)")
