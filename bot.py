...
TOKEN = "8516808804:AAFvv383G9LqDZ3BXjeMpQOc26i0JI6W6Pk"  # <<<= SUBSTITUA POR SEU TOKEN REAL
...
# 2) Resultado só depois de 65s, e só se o candle existir no minuto exato da entrada
    ...
    try:
        ...
        for i in range(len(data)-1, max(-1, len(data)-100), -1):
            ...
            try:
                ...
                # só aceita vela do mesmo minuto que a entrada
                if (candle_dt.year == target_year and
                    ...
                    candle_dt.minute == target_minute):
...
🏆 {status}
        ...
        # Se não encontrou vela exata no horário de entrada
        bot.send_message(chat_id, "🕒 Horário de entrada não encontrado no candle. Nenhum resultado foi computado.")
...
Sinal gerado AGORA → entrada no PRÓXIMO MINUTO (ex: 19:52 → entrada 19:53).
Resultado só após 65s, se o candle existir nesse minuto exato.
    """
