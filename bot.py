...
# 1) IMPEDIR MÚLTIPLOS SINAIS POR CHAT E GERAR + APAGAR MSGS
...
def send_analise_gif(chat_id):
    # apaga mensagem anterior (se tiver)
    if chat_id in user_state and "msg_id" in user_state[chat_id]:
        try:
            bot.delete_message(chat_id, user_state[chat_id]["msg_id"])
        except Exception:
    ...
    user_state[chat_id]["msg_id"] = msg.message_id  # guarda o ID da mensagem de texto


def send_sinal_msg(chat_id, coin_id, entry_time, direction, gale_time):
    # se já tem trading ativo, ignora
    ...
    # apaga a mensagem de análise antes de enviar o sinal
    if chat_id in user_state and "msg_id" in user_state[chat_id]:
        try:
            bot.delete_message(chat_id, user_state[chat_id]["msg_id"])
        except Exception:
...
def gen_signal(c):
    ...
    # evita gerar sinal se já tem um ativo
    ...
    # limpa msg_id se tiver do estado anterior
    if c.message.chat.id in user_state:
        user_state[c.message.chat.id]["msg_id"] = None
...
def choose_pair(c):
    ...
    def do_analyze():
        ...
        time.sleep(0.3)
        get_result(c.message.chat.id, direction, coin_id, entry)
...
print("🚀 Quantix Cripto Pro Ativo (1m - 65s, múltiplas criptos, layout 100%, apaga msg de análise, evita 429)")
bot.remove_webhook()
time.sleep(2)
