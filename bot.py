...
# 2) Resultado só depois de 65s a partir do minuto exato de entrada (ex: 21:17:00)
def get_result(chat_id, direction, target_time_dt):
    # target_time_dt = 21:17 (entrada marcada)
    # 65s a partir de 21:17:00
    target_start = target_time_dt.replace(second=0, microsecond=0)  # 21:17:00
    ...
    try:
        ...
        # Tenta encontrar a vela no horário de entrada (21:17) ou imediatamente após (21:18)
        for i in range(len(data)-1, max(-1, len(data)-100), -1):
            ...
            try:
                ...
                # só aceita 21:17 ou 21:18 do mesmo dia
...
🏆 {status}
        ...
        # Se não encontrou vela no loop, tenta usar a última vela
        if data:
            row = data[-1]
            if isinstance(row, list) and len(row) >= 5:
                try:
...
🏆 {status}
        ...
        # Nenhum dos dois caminhos funcionou
...
Se você pedir o sinal às 21:16:
✨ Entrada = 21:17
📅 O resultado aparece após 65s a partir de 21:17:00
🎯 Só aceita vela 21:17 ou 21:18 do mesmo dia
🎯 Se não encontrar, tenta usar a última vela
    """
