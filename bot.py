...
# 2) Resultado só depois de 65s contados a partir do minuto de entrada
def get_result(chat_id, direction, target_time_dt):
    # 20:03 → target_time_dt = 20:04 (próximo minuto)
    now = datetime.now(BR_TZ)
    # calcula: em quantas milissegundos estamos do início do minuto de entrada (ex: 20:04:00)
    seconds_in = (now - now.replace(second=0, microsecond=0)).total_seconds()
    # 65s contados desde 20:04:00
    wait = 65 - seconds_in

    # só espera se ainda não passaram 65s desde o minuto de entrada
    if wait > 0:
        time.sleep(wait)
    ...
    try:
        ...
        for i in range(len(data)-1, max(-1, len(data)-100), -1):
            ...
            try:
                ...
                # Aceita o candle de EXATAMENTE 20:04, ou 20:05 (se já estiver chegando lá)
...
🏆 {status}
        ...
        bot.send_message(chat_id, "🕒 Não foi possível encontrar o candle de entrada. Nenhum resultado foi computado.")
...
Se você pedir o sinal às 20:03:
✨ Entrada = 20:04
📅 O resultado só aparece após 65s a partir de 20:04:00
    """
...
def gerar_sinal(m):
    now = datetime.now(BR_TZ)
    # próximo minuto exato (ex: 20:03 → 20:04)
...
Aguardando 65s a partir do minuto exato para resultado...
    """
    bot.send_message(m.chat.id, text)

    # roda o resultado só depois de 65s contados desde 20:04:00 (ou equivalente)
    threading.Thread(
        target=get_result,
