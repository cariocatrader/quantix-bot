...
# GIFs (substitua se quiser os seus)
...
def start(message):
    ...
    bot.send_message(message.chat.id, "🤖 Quantix Cripto - Bot de Sinais Automático", reply_markup=kb)
...
def callback_query(call):
    try:
        ...
        if call.data in ["start", "new_signal"]:
            ...
            bot.edit_message_text("⏭️ Gale pulado!", chat_id, call.message.message_id, reply_markup=final_btn())
            if chat_id in user_state:
                del user_state[chat_id]
    except:
        pass
...
# START SEGURO
try:
    # Remove webhook SEM parâmetros extras
    requests.get(f"https://api.telegram.org/bot{TOKEN}/deleteWebhook")
    time.sleep(3)
    print("✅ Bot iniciado perfeitamente!")
