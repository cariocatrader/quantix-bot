...
def br_to_utc_timestamp(br_time_str):
    """Corrigida com validação robusta"""
    try:
...
def get_binance_candle(symbol, br_time_str, interval="1m"):
    """🔧 CORRIGIDA: Validação JSON + busca candle mais próximo"""
    try:
        target_utc_ms = br_to_utc_timestamp(br_time_str)
        end_time = target_utc_ms + 60000  # +1min (ao invés de +2min)
        url = f"https://api.binance.com/api/v3/klines?symbol={symbol}&interval={interval}&endTime={end_time}&limit=20"
        
        r = requests.get(url, timeout=10)
        data = r.json()
        
        # ✅ VALIDAÇÃO CRÍTICA: JSON deve ser lista
        if not isinstance(data, list):
            logger.error(f"❌ API não-JSON: {str(data)[:100]}")
            return None, None
        
        logger.info(f"📊 {len(data)} velas | Target: {target_utc_ms}")
        
        if not data:
            return None, None
        
        # ✅ Busca MELHOR candle (mais próximo do target)
        best_candle = None
        min_diff = float('inf')
        for candle in data:
            try:
                open_ts = int(candle[0])
                diff = abs(open_ts - target_utc_ms)
                if diff < min_diff:
                    min_diff = diff
                    best_candle = candle
        ...
        if not best_candle or min_diff > 90000:  # 90s tolerância
            logger.warning(f"❌ Candle distante: {min_diff/1000:.0f}s")
            return None, None
        
        o, c = float(best_candle[1]), float(best_candle[4])
        logger.info(f"✅ {br_time_str} ({interval}): O:{o:.4f}→C:{c:.4f} (diff:{min_diff/1000:.0f}s)")
        return o, c
...
def get_result(symbol, direction, br_time_str, interval="1m"):
    ...
    if o is None:
        logger.warning(f"⚠️ Sem candle → LOSS automático")
        return "LOSS"
...
def process_trade(chat_id, coin_id, direction, entry_time, exp, is_gale=False):
    """🔧 OTIMIZADA: Tempo futuro limitado + validação robusta"""
    def check():
        try:
            ...
            # ✅ LIMITE: máximo 6min no futuro
            if seconds_to_wait > 360:
                logger.error(f"❌ Tempo futuro demais: {seconds_to_wait}s → LOSS")
            ...
            logger.error(f"❌ process_trade erro: {e}")
            send_result_final(chat_id, SYMBOLS[coin_id], direction, "LOSS", entry_time, is_gale, interval)
...
def test(m):
    symbol = "BTCUSDT"
    br_time = get_br_time()
    logger.info(f"🔍 Test {br_time}: {br_to_utc_timestamp(br_time)}")
    o, c = get_binance_candle(symbol, br_time)
    bot.send_message(m.chat.id, f"🔧 Test {br_time}: O:{o} C:{c}")
...
print(f"🚀 QUANTIX v2.1 CORRIGIDO {get_br_time()} - Sem erros JSON + Gale PERFEITO!")
