...
# === COINGECKO (API /coins/{id}/ohlc, 1 chamada por sinal) ===
...
# 1) Análise: 1 chamada CoinGecko, 5 velas recentes
def analyze(coin_id):
...
# 2) Pega o candle que bate com o horário de entrada (BR) – ou o anterior imediato
    ...
    try:
        ...
        for i in range(len(data)-1, max(-1, len(data)-100), -1):
            ...
            try:
                ...
                # Aceita exato ou 1 minuto antes (candle fechado)
                if (candle_dt.year == target_time_dt.year and
                    candle_dt.month == target_time_dt.month and
                    candle_dt.day == target_time_dt.day and
                    candle_dt.hour == target_time_dt.hour and
                    candle_dt.minute in [target_time_dt.minute, target_time_dt.minute - 1]):
...
def get_result(coin_id, direction, entry_time_dt):
    """
    Tenta pegar o candle de entrada ou o imediatamente anterior.
    Se não conseguir, retorna None (sem resultado).
    """
...
# 3) Tempo de entrada e gale (1m ou 5m) no fuso BR
...
# 6) Processa trade após 65s ou 310s
...
# 7) Gera sinal (1 sinal por vez, 45s entre sinais)
def run_signal(chat_id, coin_id, exp):
    ...
    # 45s entre sinais (bem abaixo do limite de ~30 calls/min)
    if not hasattr(run_signal, "last_call"):
        run_signal.last_call = datetime.now() - timedelta(seconds=45)
    ...
    try:
        with open(ANALYSIS_GIF, 'rb') as gif:
            sent_msg = bot.send_animation(chat_id, gif, caption=text_analise, timeout=30)
...
print(f"🚀 QUANTIX ATIVO {get_br_time()} - 1 chamada CoinGecko, 45s entre sinais")
bot.remove_webhook()
time.sleep(2)
