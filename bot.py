...
# =========================
# ANALYZE - TENDÊNCIA COM BYBIT V5 (sem CoinGecko, com retries)
# =========================
    ...
    for tentativa in range(3):
        try:
            ...
            if r.status_code == 403:
                print("❌ Bybit 403 (CDN/Rate limit), tentando novamente...")
                time.sleep(10)
                continue

            if r.status_code != 200:
                print(f"❌ Bybit HTTP {r.status_code} em analyze_bybit")
                time.sleep(3)
                continue
            ...
            closes = [float(k[4]) for k in klines[-100:] if len(k) >= 5]
            if len(closes) < 10:
                return "COMPRA"
            ...
            print("Erro analyze_bybit (tentativa):", e)
            time.sleep(3)

    print("❌ Analyze_bybit falhou totalmente, usando fallback COMPRA")
    return "COMPRA"


# =========================
# GET_RESULT_RAW - USANDO BYBIT V5 1m (CANDLE CERTO, RESULTADO CERTO)
# =========================
def get_result_raw(direction, coin_id, target_time_dt):
    ...
    while True:
        ...
        try:
            ...
            if r.status_code == 403:
                print("❌ Bybit 403 (CDN/Rate limit) em get_result_raw, tentando novamente...")
                time.sleep(10)
                continue
...
# =========================
# MENU PARIDADES (corrigido: inicializa user_state[chat_id] antes)
# =========================
