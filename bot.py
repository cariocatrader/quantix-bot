...
# Mapeamento de ativos (sem binance, só IDs) 
SYMBOLS = {
...
# 1) Análise: 1 chamada, 5 velas recentes
def analyze(coin_id):
    ...
    try:
        ...
        if r.status_code != 200:
            print(f"❌ CoinGecko HTTP {r.status_code} para {coin_id} (analyze)")
            return "COMPRA"
...
# 2) Pega exatamente o candle que corresponde ao horário de entrada (BR time)
    ...
    try:
        ...
        for i in range(len(data)-1, max(-1, len(data)-100), -1):
            ...
            try:
                ...
                # Verifica se o candle fechou no mesmo minuto que o horário de entrada
...
# 3) Tempo de entrada e gale (1m ou 5m) no fuso do BR
...
# 6) Processa o trade (só registra resultado após 65/310s)
...
# 7) Gera o sinal (1 sinal por vez, 30s entre sinais)
def run_signal(chat_id, coin_id, exp):
    # Evita dois sinais ao mesmo tempo
    ...
    # 30s entre sinais pra ficar longe do limite do CoinGecko
    if not hasattr(run_signal, "last_call"):
        run_signal.last_call = datetime.now() - timedelta(seconds=30)
...
print(f"🚀 QUANTIX ATIVO {get_br_time()} - 1 chamada CoinGecko, 30s entre sinais")
bot.remove_webhook()
time.sleep(2)
