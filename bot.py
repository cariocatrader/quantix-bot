...
def get_candle_exact(symbol, interval="1m"):
    try:
        time.sleep(0.2)  # Rate limit CRUCIAL
        r = requests.get(f"https://api.binance.com/api/v3/klines?symbol={symbol}&interval={interval}&limit=10", timeout=15)
        ...
        data = r.json()
        if len(data) < 2:
            print(f"❌ Poucos dados {symbol}")
            return None, None, None
        
        # ÚLTIMA VELA FECHADA (sempre data[-2])
        ...
        print(f"✅ VELA {symbol}: {candle_time} O={o:.4f} C={c:.4f}")
        return o, c, candle_time
        
    except Exception as e:
        print(f"❌ ERRO {symbol}: {str(e)[:50]}")
        return None, None, None
...
def get_result(symbol, direction):
    ...
    print(f"🏆 RESULTADO {symbol}: {actual_time} O={o:.4f} C={c:.4f} → {result}")
    return result
...
def process_trade(chat_id, coin_id, direction, entry_time, exp, is_gale=False):
    def check():
        ...
        # Máximo 3 tentativas com delay maior
        for attempt in range(3):
            result = get_result(symbol, direction)
            if result is not None:
                break
            if attempt < 2:
                print(f"⏳ Tentativa {attempt+2}/3 - aguardando 15s...")
                time.sleep(15)
