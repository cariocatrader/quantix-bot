...
# Garantir que coin_id é exatamente o da CoinGecko
VALID_COIN_IDS = set(SYMBOLS.keys())  # "bitcoin", "ethereum", etc.

user_state = {}

# === ANALISE (1 CHAMADA COINGECKO) ===
def analyze(coin_id):
    """
    1 requisição para 2 dias de OHLC, pega 5 velas e decide COMPRA/VENDA.
    """
    ...
    try:
        params = {"vs_currency": "usd", "days": "2"}  # days como string, como exige API
...
# === CANDLE EXATO DO HORÁRIO DE ENTRADA ===
...
⏱ {entry_time}
🎯 {direction}
📡 API CoinGecko indisponível (400 / 500)
...
def run_signal(chat_id, coin_id, exp):
    ...
    # Delay entre sinais para evitar 429
...
print(f"🚀 QUANTIX ATIVO {get_br_time()} - 1 chamada CoinGecko por sinal (tratamento 400)")
bot.remove_webhook()
time.sleep(2)
