...
# === ANALISE COM 1 CHAMADA E 5 VELAS ===
def analyze(coin_id):
    """
    1 requisição à CoinGecko e pega 5 velas mais recentes para decidir COMPRA/VENDA.
    Isso reduz drasticamente o risco de 429.
    """
    try:
        ...
        # Pega até 5 velas mais recentes
        closes = []
        ...
        print(f"❌ Erro analyze {coin_id}: {str(e)[:50]}")
        return "COMPRA"

# === BUSCA CANDLE EXATO DO HORÁRIO DE ENTRADA (1 REQUISIÇÃO) ===
def get_candle_exact_by_time(coin_id, target_time_dt):
    """
    1 chamada só para buscar o candle exato do horário de entrada.
    """
    try:
        params = {"vs_currency": "usd", "days": "2"}
        r = requests.get(
        ...
        for i in range(len(data)-1, max(-1, len(data)-100), -1):
            ...
            # Se o candle fechou no mesmo minuto que o horário de entrada
...
def get_result(coin_id, direction, entry_time_dt):
    """
    Avalia WIN/LOSS só no candle exato do horário de entrada.
    """
...
def process_trade(chat_id, coin_id, direction, entry_time_dt, exp, is_gale=False):
    """
    Só 1 chamada a CoinGecko após o candle fechar (1m ou 5m).
    """
    def check():
...
def run_signal(chat_id, coin_id, exp):
    ...
    # Evita chamadas excessivas em sequência (dentro do limite de 30 calls/min)
    if not hasattr(run_signal, "last_call"):
        run_signal.last_call = datetime.now() - timedelta(seconds=3)
...
print(f"🚀 QUANTIX ATIVO {get_br_time()} - 1 chamada CoinGecko por sinal")
bot.remove_webhook()
time.sleep(2)
