...
def br_to_utc_timestamp(br_time_str):
    """Timestamp robusto com validação"""
    try:
        ...
        logger.error(f"❌ Timestamp erro {br_time_str}: {e}")
...
def get_binance_candle(symbol, br_time_str, interval="1m"):
    """🔧 v2.2: Preço FINAL + High/Low com tolerância 0.1%"""
    try:
        target_utc_ms = br_to_utc_timestamp(br_time_str)
        end_time = target_utc_ms + 60000
        url = f"https://api.binance.com/api/v3/klines?symbol={symbol}&interval={interval}&endTime={end_time}&limit=30"
        
        r = requests.get(url, timeout=10)
        data = r.json()
        
        if not isinstance(data, list) or not data:
            logger.error(f"❌ API inválida")
            return None, None, None, None
        
        # 🔍 Busca EXATA da vela (±30s)
        target_candle = None
        for candle in data:
            try:
                open_ts = int(candle[0])
                if abs(open_ts - target_utc_ms) <= 30000:
        ...
        if not target_candle:
            logger.warning(f"❌ Candle {br_time_str} não encontrada")
            return None, None, None, None
        
        o, h, l, c = map(float, [target_candle[1], target_candle[2], target_candle[3], target_candle[4]])
        logger.info(f"📊 {br_time_str}: O:{o:.4f} H:{h:.4f} L:{l:.4f} C:{c:.4f}")
        return o, c, h, l
...
def get_result(symbol, direction, br_time_str, interval="1m"):
    """🎯 LÓGICA MELHORADA: Close + High/Low com 0.1% tolerância"""
    result = get_binance_candle(symbol, br_time_str, interval)
    if result[0] is None:
        logger.warning("⚠️ Sem candle → LOSS")
        return "LOSS"
    
    o, c, h, l = result
    tolerance = o * 0.001  # 0.1%
    
    if direction == "COMPRA":
        # WIN se fechou verde OU alta favorável
        win = (c > o) or (h > o + tolerance)
    else:  # VENDA
        # WIN se fechou vermelho OU baixa favorável
        win = (c < o) or (l < o - tolerance)
    
    logger.info(f"🤖 {direction}: O:{o:.4f} C:{c:.4f} → {'✅WIN' if win else '❌LOSS'}")
...
def process_trade(chat_id, coin_id, direction, entry_time, exp, is_gale=False):
    """Processo robusto com limite temporal"""
    def check():
        try:
            ...
            wait_until = entry_full + timedelta(seconds=interval_secs) + timedelta(seconds=5)
            
            seconds_to_wait = max(0, (wait_until - now_dt).total_seconds())
            
            if seconds_to_wait > 360:  # 6min máximo
                logger.error(f"❌ Tempo futuro: {seconds_to_wait}s → LOSS")
                send_result_final(chat_id, SYMBOLS[coin_id], direction, "LOSS", entry_time, is_gale, interval)
                return
            
            if seconds_to_wait > 0:
                logger.info(f"⏳ Espera {seconds_to_wait:.0f}s")
                time.sleep(seconds_to_wait)
            
            symbol = BINANCE_SYMBOLS[coin_id]
            result = get_result(symbol, direction, entry_time, interval)
            logger.info(f"📊 {entry_time} ({'Gale' if is_gale else 'Principal'}): {result}")
            send_result_final(chat_id, SYMBOLS[coin_id], direction, result, entry_time, is_gale, interval)
            
            if not is_gale and result == "LOSS":
                logger.info("🚫 LOSS → Gale")
                ...
                if exp == "1":
                    ...
                    next_5 = math.ceil(minutes / 5.0) * 5
                    if next_5 >= 60:
                        next_5 = 0
            ...
            logger.error(f"❌ process_trade: {e}")
            send_result_final(chat_id, SYMBOLS[coin_id], direction, "LOSS", entry_time, is_gale, interval)
...
def test(m):
    symbol = BINANCE_SYMBOLS["bitcoin"]
    br_time = get_br_time()
    o, c, h, l = get_binance_candle(symbol, br_time)
    bot.send_message(m.chat.id, f"🔧 Test {br_time}\nO:{o} H:{h} L:{l} C:{c}")
...
def start(m):
    ...
    bot.send_message(m.chat.id, f"""🤖 *Quantix Cripto v2.2*

✅ WINs corrigidos + High/Low 0.1%
...
print(f"🚀 QUANTIX v2.2 WIN-CORRIGIDO {get_br_time()} - High/Low + 0.1% TOLERÂNCIA!")
