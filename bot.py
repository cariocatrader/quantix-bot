...
# === ANALISE COM 1 CHAMADA E days="1" (mais suave) ===
def analyze(coin_id):
    ...
    try:
        ...
        if r.status_code != 200:
            ...
            if r.status_code == 400:
                print(f"   Detalhes 400: {r.text[:200]}")
            return "COMPRA"
...
def send_animation_safe(chat_id, gif_path, caption, markup=None, attempts=2):
    """
    Tenta enviar o GIF com retry e fallback para texto se der timeout.
    """
    if attempts <= 0:
    ...
    try:
        with open(gif_path, 'rb') as gif:
            bot.send_animation(
                ...
                timeout=30  # aumenta o timeout para upload de GIF
            )
    except Exception as e:
        print(f"❌ Erro envio GIF {gif_path} (tentativa {attempts}): {str(e)}")
...
⏱ {entry_time}
🎯 {direction}
📡 API CoinGecko indisponível (429 / 500)
...
print(f"🚀 QUANTIX ATIVO {get_br_time()} - 1 chamada CoinGecko, 10s entre sinais, GIF com retry")
bot.remove_webhook()
time.sleep(2)
