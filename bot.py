...
# =========================
# ANALYZE - MESMO CÓDIGO (USA COINGECKO SÓ PARA DIREÇÃO, NÃO PARA CANDLE 1M)
# =========================
def analyze(coin_id):
    global last_call
    ...
    try:
        # Só usado para pegar tendência, não para o candle 1m de 65s
        params = {"vs_currency": "usd", "days": "1"}
        r = requests.get(f"https://api.coingecko.com/api/v3/coins/{coin_id}/ohlc", params=params, timeout=20)

        if r.status_code != 200:
            print("❌ CoinGecko HTTP em analyze")
            return "COMPRA"
...
# =========================
# GET_RESULT_RAW - USANDO BYBIT (1m)
# =========================
def get_result_raw(direction, coin_id, target_time_dt):
    ...
    while True:
        ...
        try:
            ...
            target_ts = int(target_time_dt_utc.timestamp())
            ...
            for k in klines:
                try:
                    ...
                    if start_time_dt.minute == target_time_dt_utc.minute and start_time_dt.hour == target_time_dt_utc.hour:
                        candle_found = {
                            "open": float(k[1]),
                            "close": float(k[4]),
...
def menu_paridades(chat_id):
    ...
    for key, coin_id in COIN_IDS_BYBIT.items():
        kb.add(InlineKeyboardButton(SYMBOLS[coin_id.split("USDT")[0]], callback_data=f"pair_{key}"))
...
"""
    ...
    # RESULTADO APÓS 65s (BYBIT CANDLE 1m)
