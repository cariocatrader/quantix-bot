...
# 2) Resultado só depois de 65s a partir do minuto exato de entrada (ex: 20:07:00)
def get_result(chat_id, direction, target_time_dt):
    # target_time_dt = 20:07 (entrada marcada)
    # 65s a partir de 20:07:00
    target_start = target_time_dt.replace(second=0, microsecond=0)  # 20:07:00
    target_65s = target_start + timedelta(seconds=65)
    now = datetime.now(BR_TZ)
    wait = (target_65s - now).total_seconds()

    # espera até 20:07:00 + 65s (ex: 20:08:05)
    if wait > 0:
        time.sleep(wait)
    ...
    try:
        ...
        if r.status_code != 200:
            print(f"❌ CoinGecko HTTP {r.status_code} (get_result) - {r.text[:100]}")
            bot.send_message(chat_id, "❌ ERRO ao verificar resultado no CoinGecko.")
            return

        data = r.json()
        if not data or not isinstance(data, list) or len(data) == 0:
            bot.send_message(chat_id, "❌ Sem dados de BTC para o horário de entrada.")
            return
        ...
        # Primeiro: tenta achar vela no minuto exato ou imediatamente após
        for i in range(len(data)-1, max(-1, len(data)-100), -1):
            ...
            try:
                ...
                # aceita 20:07 ou 20:08 (ou 20:09 se ainda estiver lá)
                if (candle_dt.year == target_year and
                    ...
                    candle_dt.minute <= target_minute + 1):
...
🏆 {status}
        ...
        # Se nenhum candle bateu com o horário de entrada, pega a última vela fechada
        if data:
            row = data[-1]
            if isinstance(row, list) and len(row) >= 5:
                try:
...
🏆 {status}
                    ...
                    print(f"❌ Erro processando última vela: {str(e)[:80]}")

        # Se mesmo assim não conseguiu, sem inventar resultado
        bot.send_message(chat_id, "🕒 Sem vela clara para o horário de entrada. Resultado não computado.")
...
Se você pedir o sinal às 20:06:
✨ Entrada = 20:07
📅 O resultado só aparece após 65s a partir de 20:07:00
🎯 Se encontrar o candle 20:07/20:08 → mostra WIN/LOSS
🎯 Se não encontrar, usa a última vela para não ficar sem resultado
    """
...
def gerar_sinal(m):
    now = datetime.now(BR_TZ)
    # próximo minuto exato: 20:06 → 20:07
...
Resultado após 65s a partir do minuto exato.
    ...
    # roda o resultado só depois de 65s contados desde 20:07:00 (ou equivalente)
    threading.Thread(
        target=get_result,
