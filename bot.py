...
# =========================
# GET_RESULT_RAW - SÓ DECIDE WIN/LOSS
# =========================
    ...
    try:
        ...
        for i in range(len(data)-1, max(-1, len(data)-100), -1):
            ...
            try:
                ...
                if (candle_dt.year == target_year and
                    ...
                    print(f"✅ CANDLE ENCONTRADO: {candle_time} | O={o:.2f} C={c:.2f} | RESULTADO: {res}")
                    return res
        ...
        if data:
            ...
            if isinstance(row, list) and len(row) >= 5:
                ...
                is_win = (direction == "COMPRA" and c > o) or (direction == "VENDA" and c < o)
                res = "WIN" if is_win else "LOSS"
                print(f"🔁 Fallback: {candle_time} | O={o:.2f} C={c:.2f} | RESULTADO: {res}")
                return res
...
"""
    ...
    # --- RESULTADO ENTRADA PRINCIPAL (sem WIN/LOSS por conta própria)
...
print("🚀 QUANTIX PRO ATIVO (65s, múltiplas criptos, GIF no final, botão novo sinal)")
