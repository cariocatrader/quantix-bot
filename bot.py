...
if not TOKEN:
    TOKEN = "8516808804:AAFvv383G9LqDZ3BXjeMpQOc26i0JI6W6Pk"  # só para teste; no Railway usa env
...
# 5 criptos mais líquidas (BTC, ETH, DOT, AVAX, SOL)
COIN_IDS = {
    "btc": "bitcoin",
    "eth": "ethereum",
    "dot": "polkadot",
...
# Chave global para cada chat_id
user_state = {}  # {chat_id: {"msg_id": 123, "coin_id": "...", ...}}
...
def menu_paridades(chat_id):
    ...
    for key, coin_id in COIN_IDS.items():
        kb.add(telebot.types.InlineKeyboardButton(
            f"{SYMBOLS[coin_id].split()[0]}", callback_data=f"pair_{key}")
        )
    msg = bot.send_message(chat_id, text, reply_markup=kb)
...
def send_analise_gif(chat_id):
    ...
    text = "🔍 Aguarde enquanto o Quantix está analisando o gráfico..."
    msg = bot.send_message(chat_id, text)
    try:
        with open(ANALISE_GIF, 'rb') as gif:
            bot.send_animation(chat_id, gif, timeout=30)
    except Exception as e:
...
🎯 Direção: {direction}
    ...
    threading.Thread(
        target=get_result,
        args=(chat_id, direction, coin_id, entry_time_dt),
        daemon=True
    ).start()
...
def choose_pair(c):
    ...
    # em thread para não travar o bot
    def do_analyze():
        ...
        # chamada final para get_result no final da entrada
        threading.Thread(
            target=get_result,
...
def gerar_sinal(m):
    # Este comando só funciona se o usuário ainda não tiver botão interativo rodando
    # Em produção, é opcional, pois o fluxo é todo com botões
    start_menu(m.chat.id)


print("🚀 Quantix Cripto Pro Ativo (1m - 65s, múltiplas criptos)")
bot.remove_webhook()
time.sleep(2)
