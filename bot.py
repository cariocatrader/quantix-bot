...
def analyze_kraken(coin_id):
    ...
    # Recua 10 minutos para garantir histórico suficiente
    now = datetime.now()
    ...
    for tentativa in range(3):
        try:
            ...
            if len(closes) < 10:
                print("⚠️ analyze_kraken com poucos closes, usando tentativa...")
                time.sleep(3)
                continue
...
def get_result_raw(direction, coin_id, target_time_dt):
    ...
    # Queremos o candle 1m de target_time_dt
    target_time_dt_utc = target_time_dt.astimezone(UTC_TZ)
    target_start_s = int(target_time_dt_utc.timestamp())
    since = target_start_s - 10 * 60  # 10 minutos antes
    ...
    while True:
        ...
        wait = (target_time_dt.replace(second=0, microsecond=0) + timedelta(seconds=65) - now).total_seconds()
        if wait > 0:
        ...
        try:
            ...
            # Converter target_time_dt para o mesmo fuso
