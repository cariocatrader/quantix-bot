...
def analyze(coin_id):
    """
    1 requisição para OHLC de 1 dia e pega 5 velas recentes.
    days só aceita valores específicos.
    """
    ...
    try:
        ...
        params = {"vs_currency": "usd", "days": "1"}  # 1, 7, 14, 30, 90, 180, 365, max
...
def get_candle_exact_by_time(coin_id, target_time_dt):
    ...
    try:
        # Usa dias máximos para ter mais histórico
...
print(f"🚀 QUANTIX ATIVO {get_br_time()} - 1 chamada CoinGecko por sinal (days=1/max)")
bot.remove_webhook()
time.sleep(2)
