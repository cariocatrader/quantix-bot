...
def get_binance_candle(symbol, br_time_str, interval="1m"):
    """CORRIGIDA: Usa endTime + pega ÚLTIMA vela próxima ao target"""
    try:
        target_utc_ms = br_to_utc_timestamp(br_time_str)
        end_time = target_utc_ms + 120000  # +2min margem
        url = f"https://api.binance.com/api/v3/klines?symbol={symbol}&interval={interval}&endTime={end_time}&limit=10"
        r = requests.get(url, timeout=10)
        data = r.json()
        
        logger.info(f"📊 Velas timestamps: {[int(c[0]) for c in data]} | Target: {target_utc_ms}")
        
        if not data:
            logger.warning(f"❌ Sem dados para {br_time_str}")
            return None, None
        
        # Última vela (mais recente fechada)
        last_candle = data[-1]
        open_ts = int(last_candle[0])
        if abs(open_ts - target_utc_ms) > 90000:  # ±1.5min tolerância
            logger.warning(f"❌ Mismatch ts {open_ts} vs {target_utc_ms} (>90s)")
            return None, None
        
        o, c = float(last_candle[1]), float(last_candle[4])
        logger.info(f"✅ Candle {br_time_str} ({interval}): ts={open_ts}, O:{o:.4f} → C:{c:.4f}")
        return o, c
    except Exception as e:
        logger.error(f"❌ Erro candle {br_time_str}: {e}")
        return None, None

def get_result(symbol, direction, br_time_str, interval="1m"):
    o, c = get_binance_candle(symbol, br_time_str, interval)
    if o is None:
        logger.warning(f"⚠️ Sem candle → LOSS automático para {br_time_str}")
        return "LOSS"
...
def process_trade(chat_id, coin_id, direction, entry_time, exp, is_gale=False):
    def check():
        ...
        # Espera candle fechar + 5s (mais segurança)
        ...
        wait_until = entry_full + timedelta(minutes=1 if exp=="1" else 5, seconds=5)
        ...
        logger.info(f"📊 {entry_time} ({'Gale' if is_gale else 'Entrada'} {interval}): {result}")
        
        if is_gale:
            send_result_final(chat_id, SYMBOLS[coin_id], direction, result, entry_time, True, interval)
        else:
            if result == "WIN":
                send_result_final(chat_id, SYMBOLS[coin_id], direction, result, entry_time, False, interval)
            else:
                logger.info("🚫 LOSS na entrada - indo para Gale")
                send_gale_notification(chat_id, SYMBOLS[coin_id], direction)
                
                # Gale na próxima vela do mesmo interval
                now_br = datetime.now(BR_TZ)
...
print(f"🚀 QUANTIX ATIVO CORRIGIDO {get_br_time()} - Matching de candle fixo!")
