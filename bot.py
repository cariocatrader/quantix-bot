...
# 2) Resultado só depois de 65s a partir do minuto exato de entrada
def get_result(chat_id, direction, target_time_dt):
    ...
    try:
        ...
        # Tenta encontrar a vela no horário de entrada (ou próximo)
        for i in range(len(data)-1, max(-1, len(data)-100), -1):
            ...
            try:
                ...
                if (candle_dt.year == target_year and
                    ...
                    candle_dt.minute <= target_minute + 2):
...
✨ Entrada = 20:07
📅 O resultado deve aparecer após 65s a partir de 20:07:00
🎯 Sempre envia alguma resposta após 65s (WIN/LOSS, mensagem de erro ou sem vela)
    """
