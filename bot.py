...
# =========================
# GET_RESULT_RAW - ATÉ 120s, SEM ESTOURAR API (429 CONTROLADO)
# =========================
def get_result_raw(direction, coin_id, target_time_dt):
    ...
    max_wait = 120.0  # até 120s de espera
    intervalo = 10  # 10s entre consultas
    ...
    while True:
        ...
        if wait > 0:
            time.sleep(wait)  # só até o 65s + 120s total
        ...
        try:
            ...
            if r.status_code == 429:
                print("❌ CoinGecko 429 – limite de requisições – esperando 30s")
                time.sleep(30)
                continue
    ...
    print(f"⏰ Timeout 120s atingido buscando candle 1m {target_time_dt.strftime('%H:%M')}, tratando como LOSS.")
    return "LOSS"
...
print("🚀 QUANTIX PRO ATIVO (sem Gale 1, análise 5m–15m–100m, até 120s de espera por candle 1m, com 429 controlado)")
