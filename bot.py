...
# 2) Pega o candle que bate com o horário de entrada (ou 1 minuto antes)
    ...
    try:
        ...
        for i in range(len(data)-1, max(-1, len(data)-100), -1):
            ...
            try:
                ...
                # Aceita exato ou 1 minuto antes
...
# 6) Processa trade: SÓ depois de 65/310s o bot fala WIN/LOSS ou “Price não verificado”
def process_trade(chat_id, coin_id, direction, entry_time_dt, exp, is_gale=False):
    def check():
        ...
        # Só manda WIN/LOSS ou mensagem de erro APOS o tempo de 65/310s
        if result is None:
...
def run_signal(chat_id, coin_id, exp):
    ...
    # 45s entre sinais (muito abaixo do limite de 30 calls/min)
...
print(f"🚀 QUANTIX ATIVO {get_br_time()} - 1 chamada CoinGecko, 45s entre sinais, resultado só após 65/310s")
bot.remove_webhook()
time.sleep(2)
