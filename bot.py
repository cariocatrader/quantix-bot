    ...
    try:
        ...
        if r.status_code != 200:
            ...
            if r.status_code == 429:
                bot.send_message(chat_id, "❌ ERRO 429: limite de requisições excedido no CoinGecko. Aguarde...")
...
def get_result(chat_id, direction, coin_id, target_time_dt):
    ...
    # evitar sequência muito rápida de chamadas
    time.sleep(0.5)
    ...
    try:
        ...
        for i in range(len(data)-1, max(-1, len(data)-100), -1):
            ...
            try:
                ...
                # O candle que deve ser usado: mesmo minuto da entrada, OU o PRÓXIMO 1m
                # entrada 21:34 → verificado 21:34 ou 21:35
                target_minute_target = target_minute
                if (candle_dt.year == target_year and
                    ...
                    candle_dt.minute in [target_minute_target, target_minute_target + 1]):
                    # CORRIGIR aqui: se btc para cima, é COMPRA, se para baixo, VENDA
...
🏆 {status}
        ...
        # Se não achar a vela alinhada com o minuto correto, usa a última APENAS COMO fallback
        if data:
            row = data[-1]
            if isinstance(row, list) and len(row) >= 5:
                try:
...
def start_menu(chat_id):
    # apaga qualquer mensagem de start anterior
    ...
    user_state[chat_id] = {"stage": "start", "msg_id": msg.message_id, "trading": False}


def menu_paridades(chat_id):
    # apaga mensagem de start antes de mostrar paridades
    if chat_id in user_state and "msg_id" in user_state[chat_id]:
        try:
            bot.delete_message(chat_id, user_state[chat_id]["msg_id"])
        except Exception:
...
def send_analise_gif(chat_id):
    # apaga mensagem de escolha de paridade
    if chat_id in user_state and "msg_id" in user_state[chat_id]:
        try:
            bot.delete_message(chat_id, user_state[chat_id]["msg_id"])
        except Exception:
    ...
    try:
        with open(ANALISE_GIF, 'rb') as gif:
            bot.send_animation(chat_id, gif, caption="", timeout=30)
    except Exception as e:
        print(f"❌ Erro enviando GIF analise.gif: {e}")

    user_state[chat_id]["stage"] = "analyzing"
    user_state[chat_id]["msg_id"] = msg.message_id  # só o ID da mensagem de texto


def send_sinal_msg(chat_id, coin_id, entry_time, direction, gale_time):
    if user_state.get(chat_id, {}).get("trading", False):
        bot.send_message(chat_id, "❌ Já existe um sinal em andamento. Aguarde até o resultado.")
        return

    # apaga mensagem de análise (incluindo o GIF de contexto)
    if chat_id in user_state and "msg_id" in user_state[chat_id]:
        try:
            bot.delete_message(chat_id, user_state[chat_id]["msg_id"])
        except Exception:
...
🏆 {emoji} {result_text}
    ...
    # 1) Envio do GIF com a mensagem de resultado
    if result_text == "WIN":
        path = WIN_GIF
    else:
        path = LOSS_GIF
    ...
    try:
        ...
        print(f"❌ Erro enviando GIF no resultado {path}: {e}")
        bot.send_message(chat_id, text, reply_markup=kb)

    user_state[chat_id] = {"stage": "finished", "msg_id": None, "trading": False}
...
def gen_signal(c):
    ...
    if user_state.get(c.message.chat.id, {}).get("trading", False):
        bot.send_message(c.message.chat.id, "❌ Já existe um sinal em andamento. Aguarde até o resultado.")
        return
